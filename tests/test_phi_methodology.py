...
def test_sls_reduction_halves_dynamic_increment_per_member():
    raw, env_a = _combine_raw({"S1": 1.25, "S2": 1.10}, phi_calc=1.25)
    params = _combine_params(phi_sls_reduction=True)
    ...
    assert results["phi_sls_reduced"] is True
...
def test_sls_without_reduction_keeps_full_phi():
    ...
    assert results["phi_sls_reduced"] is False


def test_sls_reduction_applies_to_manual_phi():
    raw, env_a = _combine_raw({}, phi_calc=1.0)
    params = _combine_params(phi_mode="Manual", phi=1.20, phi_sls_reduction=True)
...
def test_uls_unaffected_by_sls_reduction_flag():
    raw, env_a = _combine_raw({}, phi_calc=1.20)
    params = _combine_params(phi_sls_reduction=True)
...
def test_cache_key_excludes_sls_flag_but_includes_linf_mode():
    params = data.get_def()
    filtered = solver.solver_cache_params(params)

    assert "phi_sls_reduction" not in filtered
